...
public:
    ...
    virtual ssize_t pread(void *buf, size_t count, off_t offset) override {
        ...
        if (batch_enable) {
            ...
            batch_src_buf = (unsigned char *)malloc(batch_src_cap);
            batch_src_lens = (size_t *)malloc(sizeof(size_t) * max_batch);
            batch_dst_lens = (size_t *)malloc(sizeof(size_t) * max_batch);
            if (!batch_src_buf || !batch_src_lens || !batch_dst_lens) {
                free(batch_src_buf);  batch_src_buf = nullptr;
                free(batch_src_lens); batch_src_lens = nullptr;
                free(batch_dst_lens); batch_dst_lens = nullptr;
            ...
            if (can_batch) {
                ...
                if (batch_src_pos + block.compressed_size > batch_src_cap && batch_count > 0) {
                    if (flush_batch(batch_src_buf, batch_src_lens, batch_dst_base,
                                    batch_count) != 0) {
                        free(batch_src_buf); free(batch_src_lens); free(batch_dst_lens);
...
int zfile_decompress(IFile *src, IFile *dst) {
    ...
    size_t block_size = 1024*1024; //file->m_ht.opt.block_size;
