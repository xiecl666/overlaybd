...
static CpaPhysicalAddr v2p(void *va) {
    CpaPhysicalAddr pa = (CpaPhysicalAddr)qaeVirtToPhysNUMA(va);
    return pa ? pa : (CpaPhysicalAddr)(uintptr_t)va;
}
...
int qat_init(LZ4_qat_param *p) {
    ...
    /* Find which slot, skip StartInstance if already started. */
    int my_idx = -1;
    { photon::scoped_lock _l(g_init_mtx);
      for (int i = 0; i < g_num_instances; i++) {
    ...
    if (my_idx >= 0 && g_inst_started && !g_inst_started[my_idx]) {
        if (cpaDcStartInstance(p->inst, 0, NULL) != CPA_STATUS_SUCCESS) {
            ...
            { photon::scoped_lock _l(g_init_mtx); sal_user_stop_locked(); }
            p->inst = nullptr;
            return -1;
        }
